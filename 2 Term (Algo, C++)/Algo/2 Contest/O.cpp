...
std::istream& operator>>(std::istream& is, Graph& graph) {
  is >> graph.n >> graph.m;
  graph.edges.resize(graph.n + 1 + graph.m);
  int cnt = graph.n + 1;
  for (int i = 0; i < graph.m; ++i) {
    int first, second, third;
    is >> first >> second >> third;
    graph.edges[cnt].emplace_back(first);
    graph.edges[cnt].emplace_back(second);
    graph.edges[cnt].emplace_back(third);
    graph.edges[first].emplace_back(cnt);
    graph.edges[second].emplace_back(cnt);
    graph.edges[third].emplace_back(cnt);
    ++cnt;
  }
  return is;
}
...
void DfsVisit(const Graph& graph, int& time, std::vector<int>& color, const int& v, std::vector<int>& articulation,
  ...
  for (const auto u : graph.edges[v]) {
    ...
    if (color[u] == 0) {
      ...
      if (!flag && time_in[v] <= time_out[u] && v > graph.n) {
        articulation[v] = 1;
      }
    }
  }
  if (flag && n > 1 && v > graph.n) {
    articulation[v] = 1;
  }
  color[v] = 2;
}
...
void Dfs(const Graph& graph, int& time, std::vector<int>& articulation, std::vector<int>& time_in,
  ...
  std::vector<int> color(graph.n + 1 + graph.m, 0);
  for (int i = 1; i < graph.n + 1 + graph.m; ++i) {
...
int main() {
  ...
  std::cin >> graph;
  int time = 0;
  std::vector<int> articulation(graph.n + 1 + graph.m), time_in(graph.n + 1 + graph.m, 0),
      time_out(graph.n + 1 + graph.m, 0);
